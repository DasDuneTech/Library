...
popNamedRange = async (sheets, sheet) => {
    ...
    tagsFlt.map((tag) => {
        ...
        if (nr.length == 0) {
            ...
            nr2.addNamedRange.namedRange.range.startRowIndex = row - 1;
            nr2.addNamedRange.namedRange.range.endRowIndex = row;
            nr2.addNamedRange.namedRange.range.startColumnIndex = 1;
            nr2.addNamedRange.namedRange.range.endColumnIndex = 2;
            nr2s.push(nr2)
        }
